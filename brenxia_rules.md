...
### ① 프로젝트 생성 및 기획 수립 단계
1. **인프라 개설 (명시적 사용자 입력)**: 사용자가 구글 챗을 통해 새 프로젝트를 지시할 때 직접 입력한 **광고주명, 브랜드명, 프로젝트명** 정보만을 100% 기준으로 삼아 프로젝트 전용 **구글 챗 스페이스(Space)**와 **구글 드라이브(Drive) 폴더**(`[광고주]/[브랜드]/[프로젝트]`)를 안전하게 자동 생성합니다.
2. **RFP 업로드 및 보조 분석**: 인프라 개설 완료 후, 대표님이나 실무자가 드라이브 폴더에 **제안요청서(RFP)/기획 브리프** 파일을 업로드하면 PM 에이전트가 이를 보조적으로 정밀 분석합니다.
3. **업무 목록 및 일정 제안**: RFP 분석 결과를 토대로 필수 산출물과 기획-제작-디자인 단계별 권장 일정표를 자동 생성하여 구글 스프레드시트에 기입합니다. (사용자는 구글 챗 또는 스프레드시트에서 수동 조정 가능)
