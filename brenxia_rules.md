...
2. **인프라 개설 및 열람 범위 제한**: 관리자가 개설을 승인하고 **열람 범위(전체 공개 / 특정 팀 / 지정 사용자 등)**를 선택하면, PM 에이전트는 입력된 **광고주명, 브랜드명, 프로젝트명**을 기준으로 전용 **구글 챗 스페이스(Space)**와 **구글 드라이브(Drive) 폴더**(`[광고주]/[브랜드]/[프로젝트]`)를 자동 생성하고, 구글 드라이브 API를 통해 지정된 권한에 맞춰 열람/공유 권한을 자동으로 차단 및 설정합니다.
3. **RFP 업로드 및 보조 분석**: 인프라 개설 완료 후, 담당 PM이 드라이브 폴더에 **제안요청서(RFP)/기획 브리프** 파일을 업로드하면 PM 에이전트가 이를 보조적으로 정밀 분석합니다. (프로젝트 생성 당시에 파일이 준비되지 않은 경우, PM 에이전트는 스페이스가 개설된 후 실무자들에게 파일 업로드를 안내하고 업로드 즉시 감지하여 분석을 진행합니다.)
